...
default_domain_count="$(jq -r '[.domains[] | select((.mode // "default") as $mode | $mode != "discover" and $mode != "deploy" and $mode != "opensource" and $mode != "content")] | length' "$DOMAINS_FILE")"
default_lenses="$(jq -r '[.domains[] | select((.mode // "default") as $mode | $mode != "discover" and $mode != "deploy" and $mode != "opensource" and $mode != "content") | .lenses | length] | add' "$DOMAINS_FILE")"
